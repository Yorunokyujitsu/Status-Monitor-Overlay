...
public:
    ...
    virtual tsl::elm::Element* createUI() override {
		...
		auto Status = new tsl::elm::CustomDrawer([this](tsl::gfx::Renderer *renderer, u16 x, u16 y, u16 w, u16 h) {
			...
			if (!Initialized) {
				...
				for (std::string key : tsl::hlp::split(settings.show, '+')) {
					if (!key.compare("CPU") && !(flags & 1 << 0)) {
						...
						text_width += TEMP_dimensions.first;
						entry_count += 1;
						flags |= 1 << 3;
					}
			...
			for (std::string key : tsl::hlp::split(settings.show, '+')) {
				if (!key.compare("CPU") && !(flags & 1 << 0)) {
					auto dimensions_s = renderer->drawString("ㅤCPU", false, offset, base_y+fontsize, fontsize, renderer->a(settings.catColor));
					...
					offset += CPU_dimensions.first + margin_4;
					flags |= 1 << 0;
				}
				else if (!key.compare("GPU") && !(flags & 1 << 1)) {
					...
					offset += GPU_dimensions.first + margin_1;
					flags |= 1 << 1;
				}
				else if (!key.compare("RAM") && !(flags & 1 << 2)) {
					...
					offset += RAM_dimensions.first + margin_1;
					flags |= 1 << 2;
				}
				else if (!key.compare("TEMP") && !(flags & 1 << 3)) {
					...
					offset += TEMP_dimensions.first + margin_2;
					flags |= 1 << 3;
				}
				else if (!key.compare("FAN") && !(flags & 1 << 4)) {
					...
					offset += FAN_dimensions.first + margin_1;
					flags |= 1 << 4;
				}
				else if (!key.compare("PWR") && !(flags & 1 << 5)) {
					...
					offset += PWR_dimensions.first + margin_0;
					flags |= 1 << 5;
				}
				else if (!key.compare("FPS") && !(flags & 1 << 6)) {
					auto dimensions_s = renderer->drawString("ㅤㅤFPS", false, offset, base_y+fontsize, fontsize, renderer->a(settings.catColor));
					uint32_t offset_s = offset + dimensions_s.first + margin;
					renderer->drawString(FPS_var_compressed_c, false, offset_s, base_y+fontsize, fontsize, renderer->a(settings.textColor));
					offset += FPS_dimensions.first + margin_3;
					flags |= 1 << 6;
				}
			}
		});
