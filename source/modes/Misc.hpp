    ...
    virtual tsl::elm::Element* createUI() override {
		...
		auto Status = new tsl::elm::CustomDrawer([this](tsl::gfx::Renderer *renderer, u16 x, u16 y, u16 w, u16 h) {
			...
			if (R_SUCCEEDED(nvdecCheck | nvencCheck | nvjpgCheck)) {
				renderer->drawString("멀티미디어 클럭 속도", false, 20, 165, 20, renderer->a(0xFFFF));
			...
			if (R_SUCCEEDED(nifmCheck)) {
				renderer->drawString("네트워크", false, 20, 255, 20, renderer->a(0xFFFF));
				if (!Nifm_internet_rc) {
					if (NifmConnectionType == NifmInternetConnectionType_WiFi) {
						renderer->drawString("타입 : Wi-Fi", false, 20, 280, 18, renderer->a(0xFFFF));
						if (!Nifm_profile_rc) {
							if (Nifm_showpass)
								renderer->drawString(Nifm_pass, false, 20, 305, 15, renderer->a(0xFFFF));
							else
								renderer->drawString("Y 버튼으로 패스워드 확인", false, 20, 305, 15, renderer->a(0xFFFF));
						}
					}
					else if (NifmConnectionType == NifmInternetConnectionType_Ethernet)
						renderer->drawString("타입 : 이더넷", false, 20, 280, 18, renderer->a(0xFFFF));
				}
				else
