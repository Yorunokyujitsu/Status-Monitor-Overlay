    ...
    virtual tsl::elm::Element* createUI() override {
		...
		auto Status = new tsl::elm::CustomDrawer([this](tsl::gfx::Renderer *renderer, u16 x, u16 y, u16 w, u16 h) {
			...
			if (gameStart && *API_shared == 1) {
				...
				renderer->drawString("심도 [Depth]:", false, base_x+20, base_y+20, 20, renderer->a(settings.catColor));
				renderer->drawString(Resolutions_c, false, base_x+20, base_y+55, 18, renderer->a(settings.textColor));
				renderer->drawString("노출 [Viewport]:", false, base_x+180, base_y+20, 20, renderer->a(settings.catColor));
				...
				if (gameStart && *API_shared > 1) {
					...
					renderer->drawString("  - NVN을 사용하지 않아 호환되지 않습니다.", false, base_x, base_y+20, 18, renderer->a(0xF00F));				
				}
				else {
					renderer->drawRect(base_x, base_y, 360, 28, a(settings.backgroundColor));
					renderer->drawString("  - 실행중이거나 호환되는 게임이 없습니다.", false, base_x, base_y+20, 18, renderer->a(0xF00F));
				}
			}
		});
